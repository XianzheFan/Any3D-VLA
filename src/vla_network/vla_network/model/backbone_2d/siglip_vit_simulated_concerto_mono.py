...
SIGLIP_NAMES = {
    224: "vit_so400m_patch14_siglip_224",
    384: "vit_so400m_patch14_siglip_384",
}
...
class ConcertoSigLIPViTBackbone(Backbone2D):
    ...
    def __init__(self, config: Backbone2DConfig) -> None:
        ...
        siglip_name = SIGLIP_NAMES[config.image_size]
        siglip_model: ViT = ViT(
            timm.create_model(siglip_name, pretrained=True, num_classes=0, img_size=config.image_size)
        )
        siglip_model.eval()
        for param in siglip_model.parameters():
            param.requires_grad = False

        siglip_model.to(self.device)
        siglip_model.model = siglip_model.model.to(dtype=self.compute_dtype)
        self.siglip = siglip_model

        model_cfg = timm.data.resolve_model_data_config(self.siglip.model)
        ...
        self.image_feature_dim = self.siglip.embed_dim
    ...
    def forward(self, images: torch.Tensor, depths: Optional[torch.Tensor] = None,
        ...
        B, n_cams, n_views, C, H, W = images.shape

        if n_views == 1:
            imgs_siglip = images[:, :, 0]
        else:
            imgs_siglip = images[:, :, -1]
        imgs_siglip = imgs_siglip.reshape(B * n_cams, C, H, W)

        with torch.autocast(device_type="cuda", dtype=self.compute_dtype):
            feat_siglip = self.siglip(imgs_siglip)[:, 1:, :]  # remove CLS
            
            image_feats_flat = feat_siglip  # [B*n_cams, L, D_img]
            L = image_feats_flat.shape[1]
            image_feats_per_view = image_feats_flat.view(B, n_cams, L, -1)  # [B, n_cams, L, D_img]
            
        if transformed_pc is None or transformed_pc.numel() == 0:
            print("transformed_pc is None", flush=True)
            pc_feats_per_view = self.empty_pc_token.view(1, 1, 1, -1).expand(B, n_cams, L, -1)
            with torch.autocast(device_type="cuda", dtype=self.compute_dtype):
                fused_in = torch.cat([pc_feats_per_view.to(dtype=self.compute_dtype), image_feats_per_view], dim=-1)
                delta = self.fuse_mlp(fused_in.reshape(B * n_cams * L, -1)).reshape(B, n_cams, L, self.image_feature_dim)
                gate = torch.sigmoid(self.fuse_gate).view(1, 1, 1, 1)
                fused = image_feats_per_view + gate * self.fuse_ln(delta)
                feats_out = fused.reshape(B, n_cams * L, self.image_feature_dim)
            return feats_out
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            ...
            batch_idx = (batch_idx * n_cams + cam_id).long()
            ...
            pc_feats_per_view = mean_flat.view(B, n_cams, L, self.pc_feature_dim)  # float32
            counts_per_view = counts_flat.view(B, n_cams, L)
        ...
        with torch.autocast(device_type="cuda", dtype=self.compute_dtype):
            fused_in = torch.cat([pc_feats_per_view, image_feats_per_view], dim=-1)  # [B,n_cams,L,D_pc+D_img]
            delta = self.fuse_mlp(fused_in.reshape(B * n_cams * L, -1)).reshape(B, n_cams, L, self.image_feature_dim)
            gate = torch.sigmoid(self.fuse_gate).view(1, 1, 1, 1)
            fused = image_feats_per_view + gate * self.fuse_ln(delta)
            feats_out = fused.reshape(B, n_cams * L, self.image_feature_dim)
